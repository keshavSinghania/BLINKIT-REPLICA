...
const Navbar = () => {
  ...
  const [isDownArrow, setIsDownArrow] = useState(false)
  ...
  //handling logout inside this page
  const logOutHandler = () => {
              ...
              <section className='relative '>
                ...
                {(userName === "Guest") ?
                  ...
                  <div className='flex items-center justify-center gap-1 w-[7vw] h-[6vh] text-lg text-gray-700'>
                    <div>
                      <MdAccountCircle size={30} onClick={() => { navigate("dashboard/profile")}} className='lg:hidden md:ml-60 ml-35 md:block block' />
                    </div>
                    <p className='lg:block md:hidden hidden'>Account</p>
                    {isDownArrow ? (<IoMdArrowDropdown className='cursor-pointer' onClick={() => { setIsDownArrow(!isDownArrow) }} />)
                      : (<IoMdArrowDropup className='cursor-pointer' onClick={() => { setIsDownArrow(!isDownArrow) }} />)}
                  </div>

                }
                {/*account sectiondrop down box */}
                {
                  isDownArrow ?
                    (<div className=' bg-white absolute top-[3vw]  min-w-[13vw] min-h-[25vh] shadow rounded-lg flex items-center flex-col '>
                      {/* box content */}
                      <div className='flex justify-center flex-col w-[90%] min-h-[10vh] border-b-2 border-gray-300'>
                        <p className='font-bold'>My Account</p>
                        <div className='flex items-center gap-1'>
                          <p>{userName}</p>
                          <Link onClick={() => setIsDownArrow(!isDownArrow)} to="dashboard/profile"><FaExternalLinkAlt size={13} /></Link>
                        </div>
                      </div>
                      <div className='w-[100%] p-[1vw] text-gray-700'>
                        <Link onClick={() => setIsDownArrow(!isDownArrow)} to="dashboard/myorders"><p>My Orders</p></Link>
                        <Link onClick={() => setIsDownArrow(!isDownArrow)} to="dashboard/address"><p>Save Address</p></Link>
